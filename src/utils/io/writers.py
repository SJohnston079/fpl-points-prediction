...
class ZipfileOutputManager(BaseOutputManager):
    ...
    def execute(self):
        """Executes the Output Manager
            0) Validating the Existence of the Output path
            1) Build Metadata
        ...
        validate_output_location(self.output_path)
        metadata = self.build_metadata()
        log.info(f'Step 1: Built Metadata: {pprint.pformat(metadata, indent=2, sort_dicts=False, compact=True)}')
